...
<div class="bg-white p-6 sm:p-8 rounded-3xl shadow-sm border border-slate-100 max-w-3xl">
    <form action="{{ route('admin.mata-pelajaran.update', $mataPelajaran) }}" method="POST">
        ...
        <div class="mb-6">
            ...
            <input type="text" name="kode_mapel" id="kode_mapel" value="{{ old('kode_mapel', $mataPelajaran->kode_mapel) }}" class="w-full px-4 py-3 rounded-xl border border-slate-200 focus:border-school-primary focus:ring focus:ring-school-primary/20 transition-all outline-none font-mono" required>
            @error('kode_mapel')
        ...
        <div class="mb-6">
            ...
            <input type="text" name="nama_mapel" id="nama_mapel" value="{{ old('nama_mapel', $mataPelajaran->nama_mapel) }}" class="w-full px-4 py-3 rounded-xl border border-slate-200 focus:border-school-primary focus:ring focus:ring-school-primary/20 transition-all outline-none" required>
            @error('nama_mapel')
                <p class="mt-2 text-xs text-rose-500 font-medium">{{ $message }}</p>
            @enderror
        </div>
        ...
        <div class="mb-8">
            ...
            <select name="tingkat" id="tingkat" class="w-full px-4 py-3 rounded-xl border border-slate-200 focus:border-school-primary focus:ring focus:ring-school-primary/20 transition-all outline-none">
                ...
                <option value="10" {{ old('tingkat', $mataPelajaran->tingkat) == '10' ? 'selected' : '' }}>10 (Sepuluh)</option>
                <option value="11" {{ old('tingkat', $mataPelajaran->tingkat) == '11' ? 'selected' : '' }}>11 (Sebelas)</option>
                <option value="12" {{ old('tingkat', $mataPelajaran->tingkat) == '12' ? 'selected' : '' }}>12 (Dua Belas)</option>
            </select>
        ...
        <div class="flex justify-end gap-4 border-t border-slate-100 pt-6">
            ...
            <button type="submit" class="px-6 py-3 bg-school-primary text-white rounded-xl font-bold text-sm hover:bg-school-primary/90 transition shadow-lg shadow-school-primary/30">Update Mapel</button>
        </div>
    </form>
</div>
@endsection
